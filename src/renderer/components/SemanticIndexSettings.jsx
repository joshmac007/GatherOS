...
export function deriveSemanticIndexView(input) {
  ...
  const healthState = typeof status.status === 'string' ? status.status.toLowerCase() : '';
  const connected = Boolean(
    status.connected
      ?? (['connected', 'model-missing'].includes(healthState)
        ? true
        : status.ok ?? status.healthy ?? false),
  );
  ...
  const explicitModelInstalled = status.modelInstalled
    ?? status.model_installed
    ?? status.modelAvailable
    ?? status.model_available;
  const modelInstalledValue = explicitModelInstalled === undefined
    ? healthState === 'connected'
      ? true
      : healthState === 'model-missing'
        ? false
        : undefined
    : explicitModelInstalled;
  ...
  return {
    ...
    modelStatusLabel: modelInstalled === true
      ...
      : modelInstalled === false
        ...
        : 'Checking',
    provider,
    setupAction: typeof status.setupAction === 'string' ? status.setupAction : null,
    indexed,
    total,
    percent,
      ...
      <div className={styles.headingRow}>
        <div>
          ...
          <p className={styles.hint}>
            Your configured embedding provider indexes saved text so search can match by meaning.
          </p>
        </div>
      ...
      <div className={styles.statusCard}>
        <dl className={styles.statusGrid}>
          <div>
            ...
            <dd>{view.provider} · {view.healthLabel}</dd>
          </div>
          <div>
            <dt>Model</dt>
