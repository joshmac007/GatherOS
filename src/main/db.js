...
const MIGRATIONS = [
  ...
  // Only tweet_meta changes; save/import time, files, tags, and deletion state
  // remain untouched. Deleted rows are repaired in place, never restored.
  (database) => {
    repairMissingXTweetCreatedAt(database);
  },
  // Stable social-post identity. Existing duplicate rows stay untouched;
  // the oldest active row becomes canonical so migration never deletes user
  // data. Every future active save is protected by the partial unique index.
  (database) => {
