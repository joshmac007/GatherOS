...
const DEFERRED_SEMANTIC_REASONS = new Set([
...
function createSaveBackgroundRouter({
  ...
  async function finishClaim(record, { enrich = false, taskRouteSave = null } = {}) {
    try {
      ...
      const routeFn = taskRouteSave || backgroundRuntime.routeSave.bind(backgroundRuntime);
      const routed = await routeFn(current, { changed: enrich });
