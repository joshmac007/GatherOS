...
class VideoAnalysisUnavailableError extends Error {
  constructor(error) {
    super(error?.message || 'Structured video analysis is unavailable', { cause: error });
    this.name = 'VideoAnalysisUnavailableError';
    this.code = 'video_analysis_unavailable';
...
function videoSuggestionPrompt(input) {
  return {
    system:
      'Analyze this visual evidence from one saved video and suggest specific tags.\n\n' +
      'Return JSON only:\n' +
      '{"tags":[{"name":"concise normalized tag","confidence":"high",' +
      '"evidence":["visual","post_context"],"conflict":false}],"warnings":[]}\n\n' +
      'Rules:\n' +
      ...
      '- Use post context only when it agrees with visual evidence.\n' +
      '- Mark conflict true and omit the tag when text and visual evidence conflict.\n' +
      '- Never infer a tag from text alone.\n' +
      '- Do not repeat accepted tags listed in input context.\n' +
...
function createVideoAnalysisService({
  ...
  async function run({ job, save, sourceContext = {}, acceptedTags = [], posterPath = null } = {}) {
    ...
    try {
      ...
      raw = await structuredProvider.completeJson({
        ...videoSuggestionPrompt(input),
        imagePath,
      });
    } catch (error) {
      if (error?.code === 'invalid_video_suggestions') throw error;
      throw new VideoAnalysisUnavailableError(error);
    }
