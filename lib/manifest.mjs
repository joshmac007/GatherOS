...
function verifySource(manifest, sourceRoot) {
  ...
  const isBare = gitText(['rev-parse', '--is-bare-repository'], source) === 'true'
  if (!isBare) {
    expect(gitText(['status', '--porcelain'], source) === '', 'source reconstruction must be clean')
  }
  expect(
    gitText(['rev-parse', manifest.source.canonical_evidence_ref], source) === manifest.source.tip,
    'canonical evidence ref does not match manifest tip',
  )
...
export function loadAndVerifyManifest({ root, manifestPath = 'manifests/overlay.v1.json', source } = {}) {
  ...
  expect(
    manifest.source?.canonical_evidence_ref === 'refs/gatherlocal/evidence/overlay-v1',
    'unexpected canonical evidence ref',
  )
