...
export function treeInventory(root, { includeMtime = false, includeContent = false, exclude = () => false } = {}) {
  ...
  function visit(directory) {
    for (const name of fs.readdirSync(directory).sort()) {
      ...
      if (type === 'file') {
        entry.size = stat.size.toString()
        if (includeContent) entry.sha256 = sha256File(full)
      }
